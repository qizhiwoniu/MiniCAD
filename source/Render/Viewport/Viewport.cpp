...
{    
    ...
    {   
        ...
        {  
            ...
            { // 坐标轴
                auto axis = m_axis.BuildAxis(m_camera,  viewState.ShowAxis, target.viewport.Width, target.viewport.Height);
				if (!axis.empty())
					m_renderer.Submit(axis, screenVP, PrimitiveType::Line, true, true);
            }

            { // 原点小方块
                auto Gizmo = m_gizmo.BuildGizmo(m_camera, viewState.ShowGizmo, target.viewport.Width, target.viewport.Height);
                if (!Gizmo.empty())
